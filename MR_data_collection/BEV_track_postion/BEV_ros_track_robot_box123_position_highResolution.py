...
box2_publisher = rospy.Publisher('/turtle1/box2',Pose,queue_size=10)
box3_publisher = rospy.Publisher('/turtle1/box3',Pose,queue_size=10)
   ...
   while True:
       ...
       if np.all(ids): # if any markers were found
        ...
        #    OriginCornersCamera = getCornerInCameraWorld(marker_dimension, rvecs[originID], tvecs[originID])[0] # origin tag corners in camera space
         ...
         #   xy = found_dict_pixel_space[str(top_right)][0][2] #top right - 3
           ...
           box2 = found_dict_homography_space[str(box2_ID)][0] #the 4 corners of box1_ID
           box3 = found_dict_homography_space[str(box3_ID)][0] #the 4 corners of box1_ID

           print(getMarkerCenter(robot))
           im_out = cv2.resize(im_out, (500,500), interpolation = cv2.INTER_LINEAR)
           cv2.imshow('Warped Source Image', im_out)

           display = cv2.resize(display, (720,480), interpolation = cv2.INTER_LINEAR)
    ...
    #    pose_publisher.publish(robot_pose)
      ...
      #  box1_pose.theta = getMarkerRotation(box1)
       ...
       box2_pose.x = round(getMarkerCenter(box2)[0], 1) # round the value of 'robot_center[0]' to 4 decimal places
       box2_pose.y = round(getMarkerCenter(box2)[1], 1) # doesn't work in rqt?
      #  box2_pose.x = getMarkerCenter(box2)[0]
      #  box2_pose.y = getMarkerCenter(box2)[1]
      #  box2_pose.theta = getMarkerRotation(box2)
       box2_publisher.publish(box2_pose)

       box3_pose.x = round(getMarkerCenter(box3)[0], 1) # round the value of 'robot_center[0]' to 4 decimal places
       box3_pose.y = round(getMarkerCenter(box3)[1], 1) # doesn't work in rqt?
      #  box3_pose.x = getMarkerCenter(box3)[0]
      #  box3_pose.y = getMarkerCenter(box3)[1]
      #  box3_pose.theta = getMarkerRotation(box3)
       box3_publisher.publish(box3_pose)
